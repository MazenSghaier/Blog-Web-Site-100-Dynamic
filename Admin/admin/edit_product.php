...
if(isset($_POST['save'])){
   ...
   $update_post = $conn->prepare("UPDATE `project` SET titre = ?, resume = ?, prix = ?, status = ? WHERE id = ?");
   ...
   if(!empty($image)){
      if($image_size > 2000000){
         ...
         $update_image = $conn->prepare("UPDATE `project` SET image = ? WHERE id = ?");
...
<section class="post-editor">
   ...
   <form action="" method="post" enctype="multipart/form-data">
      ...
      <?php if($fetch_posts['image'] != ''){ ?>
         <img src="../images/<?= $fetch_posts['image']; ?>" class="image" alt="">
